...
contract lec13 {
    ...
    function callAddNumber(
        address _lec13_1Address,
        uint256 _num1,
        uint256 _num2
    ) public returns (bool, bytes memory) {
        (bool sent, bytes memory outputFromCalledFunction) = _lec13_1Address
            .call(
    ...
    function callWhoIsMsgSender(address _lec13_1Address)
        public
        returns (bool, bytes memory)
    {
        (bool sent, bytes memory outputFromCalledFunction) = _lec13_1Address
            .call(abi.encodeWithSignature("whoIsMsgSender()"));
        require(sent, "failed");
        return (sent, outputFromCalledFunction);
    }
}

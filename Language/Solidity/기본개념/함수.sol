...
contract lec2 {
    ...
    */
    ...
    function fun2(uint256 _num) external {
        a = _num;
    }

    function fun2_1() external {
        // external은 외부에서만 접근할 수 있는데, smart contract내부에 접근하기 위해서 this를 사용
        this.fun2(5);
    }

    function fun3() private returns (uint256) {
        a = 3;
        return a;
    }

    function fun3_1() public returns (uint256) {
        return fun3();
    }

    function fun4(uint256 _num) internal returns (uint256) {
        a = a + _num;
        return a;
    }

    function fun4_1() public returns (uint256) {
        return fun4(55);
    }
}

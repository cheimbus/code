...
contract safe {
    ...
    function() external payable {
        ...
        // 스마트 컨트랙트를 인스턴스화하면 가스발생이 증가 => send, transfer는 제한적인 가스 (2300)때문에
        // fallback함수가 실패하게 됨
        // 가스비용이 증가할 수 있으므로, call을 사용하는것을 권장함
...
contract lec14 {
    ...
    function callWrong(address _safeAddress)
        public
    ...
    function callWrong2(address _safeAddress)
        public
        payable
        returns (bool, bytes memory)
    {
        (bool sent, bytes memory outputFromCalledFunction) = _safeAddress
            .call
